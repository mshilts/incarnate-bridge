...
import { loadBridgeGameConfig, type BridgeGameConfig } from "./config.js";
...
function resolveGameConfig(args: string[]) {
...
function acceptedResult(packet: Record<string, unknown>, gameConfig: BridgeGameConfig) {
...
async function authenticateConnection(connection: Awaited<ReturnType<typeof connectCommandSocket>>, options: CommonOptions) {
  ...
  const signature = signPayload(
    options.keyPath,
    String(challenge[protocol.authChallengePayloadField] ?? ""),
    options.gameConfig.signingNamespace
  );
