...
async function verifyRejectedConnection(port: number, token: string, origin: string, label: string) {
...
async function verifyAuthenticatedBridgeControls(port: number, mockAi: MockAiServer) {
  ...
  ws.send("{malformed");
  await waitFor(
    () => packets.some((packet) => packet.type === "session_error" && packet.code === "invalid_browser_json"),
    "malformed browser JSON should be rejected without killing the session"
  );

  ws.send("x".repeat(70_000));
  await waitFor(
    () => packets.some((packet) => packet.type === "session_error" && packet.code === "browser_message_too_large"),
    "oversized browser message should be rejected without killing the session"
  );
  ...
  for (const command of [
    ...
    await waitFor(
      () => mockAi.received.some((packet) => packet.type === command.type),
      `expected ${command.type} to relay to server authorization`
    );
  }
...
async function assertBlockedFromAi(
  ws: WebSocket,
