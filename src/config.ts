...
const DEFAULT_BRIDGE_GAME_CONFIG: BridgeGameConfig = {
...
export function defineBridgeGameConfig(input: BridgeGameConfigInput): BridgeGameConfig {
  const protocol = {
...
export function loadBridgeGameConfig(configPath: string): BridgeGameConfig {
  const resolvedPath = expandHome(configPath);
  const raw = fs.readFileSync(resolvedPath, "utf8");
  const parsed = JSON.parse(raw) as BridgeGameConfigInput;
